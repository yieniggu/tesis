...
def get_dataset(images_dir,
  ...
  if use_synthetic:
    ...
    print("[DATASET] coco instance: {} - type {}".format(coco, type(coco)))
    image_ids = coco.getImgIds()
    #print("[DATASET] image ids: {} - type {}".format(image_ids, type(coco)))

    image_paths = []
    for image_id in image_ids:
      coco_img = coco.imgs[image_id]
      #print("[DATASET] image id: {} - coco image {} - type {}"
      #    .format(image_id, coco_img, type(coco_img)))
      image_paths.append(os.path.join(images_dir, coco_img['file_name']))
    print("[DATASET] There are {} images in the coco instance".format(len(image_paths)))
    #print("[DATASET] image paths: {} - type {}".format(image_paths, type(image_paths)))
    # create a dataset with tf.data object, from the image paths
    dataset = tf.data.Dataset.from_tensor_slices(image_paths)
    print("[DATASET] Dataset creation: ", dataset, dataset.element_spec)
    def preprocess_fn(path):
      ...
      return image
    # apply preprocessing function to the elements in dataset
    print("[DATASET] Applying preprocessing function to dataset...")
    dataset = dataset.map(map_func=preprocess_fn, num_parallel_calls=8)
    # combine consecutive elements of this dataset into batches
    print("[DATASET] Batching elements in dataset with batch size of {}"
        .format(batch_size))
    dataset = dataset.batch(batch_size)
    dataset_as_numpy_iterator = list(dataset.as_numpy_iterator())
    print("[DATASET] Dataset length after batching: {}"
        .format(len(dataset_as_numpy_iterator)))
    #print("[DATASET] Print dataset as np iterator after batching {}"
    #    .format(dataset_as_numpy_iterator))
    # repeat dataset to add elements already in dataset with count
    dataset = dataset.repeat(count=1)
    dataset_as_numpy_iterator = list(dataset.as_numpy_iterator())
    print("[DATASET] Dataset length after repeat: {}"
        .format(len(dataset_as_numpy_iterator)))
    #print("[DATASET] Print dataset as np iterator after repeat {}"
    #    .format(dataset_as_numpy_iterator))
...
def run_inference(graph_func,
  ...
  predictions = {}
  iter_times = []
  print("[INFERENCE] Initial iter times: ", iter_times, type(iter_times))
  ...
  print("[INFERENCE] Dataset from run inference: ", dataset, dataset.element_spec)
  dataset_as_numpy_iterator = list(dataset.as_numpy_iterator())
  print("[INFERENCE] Dataset length from inference: {}"
        .format(len(dataset_as_numpy_iterator)))
  #print("[INFERENCE] Print dataset as np iterator from inference {}"
  #      .format(dataset_as_numpy_iterator))
  if mode == 'validation':
    for i, batch_images in enumerate(dataset):
      print("[INFERENCE] Predicting image i: {} - shape: {} - type: {}"
            .format(i, batch_images.shape, type(batch_images)))
      start_time = time.time()
      batch_preds = graph_func(batch_images)
      end_time = time.time()
      iter_times.append(end_time - start_time)
      #print("[INFERENCE] Appending to iter times: ", iter_times, type(iter_times))
      print("[INFERENCE] Appending to iter times...")
  ...
  print("[INFERENCE] Converting to np array iter times: ", iter_times, type(iter_times), len(iter_times))
  if len(iter_times) > num_warmup_iterations:
    iter_times = iter_times[num_warmup_iterations:]
    print("[INFERENCE] Warmup iters: ", num_warmup_iterations)
    print("[INFERENCE] Iter times post warmup: ", len(iter_times), type(iter_times))
...
def eval_model(predictions, image_ids, annotation_path):
  name_map = {
      ...
      'output_1':'detection_classes',
      'output_2':'num_detections',
      'output_3':'detection_scores',
  }
  #print("[EVAL-MODEL] Predictions: {}".format(predictions))
  print("[EVAL-MODEL] Predictions keys: {}".format(predictions.keys()))
  for old_key in list(predictions.keys()):
    print("[EVAL-MODEL] Old key: {}".format(old_key))
    if old_key in name_map:
      new_key = name_map[old_key]
      print("[EVAL-MODEL] New key: {}".format(new_key))
      predictions[new_key] = predictions[old_key]
      del predictions[old_key]
  for key in predictions:
    print("[EVAL-MODEL] Key in predictions: {}".format(key))
  ...
  coco = COCO(annotation_file=annotation_path)
  print("[EVAL-MODEL] coco instance: {}".format(coco))
  coco_detections = []

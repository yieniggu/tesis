...
class YoloInferator():
    ...
    def run_inference(self, image_path=None, warmup_iters=0, threshold=0.5, iou=0.45, model_dir=None):   
    ...
    def run_inference_on_images(self, images_path, warmup_iters=0, model_dir=None, labels=None, threshold=0.3, iou=0.45):
        input_size = int(self.attributes["INPUT_SIZE"])
        labels = self.attributes["LABELS"]
        ...
        bbox_results.save_results(output_path, model_name, precision, 
                                threshold, resize=True, opencv=True)
